...
namespace App\Providers;
	...
	*/
	class BoxAPI {
		...
		public $asUser			= '';
		public function __construct($client_id , $client_secret , $token_object , $redirect_uri='') {
		...
		public function download_file($fileId){
			...
			$url = $this->build_url('/files/'.$fileId.'/content');
			return $this->get($url);

		}	
		/* Uploads a file */
		public function put_file($filePath, $name, $parent_id) {
			$url = $this->build_url('/files/content', array(), $this->upload_url);
			$attrs = array('name' => $name, 'parent' => array('id' => $parent_id));
			$cfile = curl_file_create($filePath,mime_content_type($filePath),$name);
			$params = array('attributes' => json_encode($attrs), 'file' => $cfile);
			error_log("Going to upload file ".print_r($params,true));
			return json_decode($this->post($url, $params), true);
		}
		...
		public function set_token($token_object){

			$this->token_object=$token_object;
			$this->refresh_token = $token_object->refresh_token;
		...
		public function isTokenExpired(){
			...
			$ctimestamp = time();
			error_log("Current Time is ".$ctimestamp);
			if(($this->token_object->expires-$ctimestamp)<=60){
				error_log("Token expires in ".$this->token_object->expires);
				return true;
			}
			return false; 
		}
